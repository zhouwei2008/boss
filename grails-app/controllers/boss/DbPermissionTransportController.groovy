...
class DbPermissionTransportController {
    /*def clear = {
        def rst = false;
                ...
                if(permStr.split(',').any{it==adminID}){
                  adminRoleList.add(oldRole.roleCode)
                    ...
                    for(String permIdStr : permStrArr){
                        ...
                        boolean isOK = set.addAll(sonList)
                        if(!isOK){
                            log.error("${sonList} add rolePerm failed!")
                        }
                    }
                    def rolePerms = []
                    set.each {
