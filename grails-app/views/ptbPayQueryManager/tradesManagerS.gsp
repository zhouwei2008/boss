...
<head>
    ...
    <g:set var="entityName" value="${message(code: 'tbAgentpayDetailsInfo.label', default: '收款明细')}"/>
    <title><g:message code="default.list.label" args="[entityName]"/></title>
    <script type="text/javascript">
        function filter() {
            document.forms['tradesManagerS'].submit();
        }
...
<div class="main">
    ...
    <div class="right_top">
        ...
        <h2>
         ...
         %{--<g:form name="tradesManagerS" action="tradesManagerS">--}%
        <div class="table_serch">
            <table align="center" class="right_list_table">
              <g:form name="tradesManagerS" action="tradesManagerS">
                ...
                付款银行：<g:select id="bankId" name="bankId" from="${bankList}" onChange="filter()" optionKey="${{it.BANKID}}" optionValue="${{it.BANKNAME}}" value="${params.bankId}" noSelection="${['':'--请选择--']}"  />
                收款状态：<g:select id="tradeStatus" name="tradeStatus" from="${PtbPayTrade.TradeStatusMap}" optionKey="key" optionValue="value" value="${params.tradeStatus}" noSelection="${['':'-请选择-']}" class="right_top_h2_input" onChange="filter()"/>
                <br>
                ...
                收款渠道： <g:select  name="merchantId" from="${bankChanelList}" onChange="filter()" optionKey="${{it.MERCHANTID}}" optionValue="${{it.BANKNAME+'-'+it.ALIAS_NAME+'-'+it.SERVICECODE}}" value="${params.merchantId}" noSelection="${['':'--请选择--']}" />
                <br>
                ...
                %{--交易序号：<g:textField name="tradeBatchseqno" value="${params.tradeBatchseqno}" size="10" class="right_top_h2_input" style="width:150px"/>--}%
                <br>
                ...
                <bo:hasPerm perm="${Perm.NewAgCol_Trade_Dl}"><g:actionSubmit class="right_top_h2_button_tj" action="downTrades" value="下载Excel"  onclick="return checkDate()" /></bo:hasPerm>
                &nbsp; &nbsp;
                <bo:hasPerm perm="${Perm.NewAgCol_Trade_Dl_CSV}"><g:actionSubmit class="right_top_h2_button_tj" action="downTradesCSV" value="下载CSV"  onclick="return checkDate()" /></bo:hasPerm>
            </table>
         </div>
         <div class="right_list_tablebox" style="height: 380px">
                <table align="center" class="right_list_table">
                    <tr>
                        ...
                        %{--<g:sortableColumn params="${params}" property="t.TRADE_TYPE" title="收款类型"/>--}%
                        ...
                        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                            ...
                            <td nowrap>
                                <g:if test="${ptbPayTrade.TRADE_STATUS=='2'||ptbPayTrade.TRADE_STATUS=='3'}">
                                    <bo:hasPerm perm="${Perm.NewAgCol_Trade_ReSend}"><a onclick="reSendCheckMsg('${ptbPayTrade.TRADE_ID}');">重发通知</a> </bo:hasPerm>
                                </g:if>
                            </td>
