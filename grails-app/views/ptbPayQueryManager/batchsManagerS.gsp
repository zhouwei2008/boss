...
<head>
  ...
  <title><g:message code="default.list.label" args="[entityName]"/></title>
    <script type="text/javascript">
     ...
     function query(){
        if(checkDate()){
           document.forms['myForm'].action='${request.contextPath}/ptbPayQueryManager/batchsManagerS'
     ...
     function cancelBatch(batchId){
        if(confirm("批次将被删除，交易恢复为等待收款，确定要撤消么?\n\n"+
...
<div class="main">
  <g:if test="${flash.message}">
    <div class="message">${flash.message}</div>
      ...
      <g:form name="myForm" action="batchsManagerS">
           <g:hiddenField name="batchType" value="S"/>
  <div class="right_top">
    <h1><img src="${resource(dir: 'images', file: 'ico_4.gif')}" width="16" height="15">收款批次管理</h1>
        <h2>
    ...
    <table align="center" class="right_list_table" id="test">
           ...
           收款状态：<g:select  onchange="return query()" name="batchStatus" from="${PtbPayBatch.BatchStatusMap}" optionKey="key" optionValue="value" value="${params.batchStatus}" noSelection="${['':'-请选择-']}" class="right_top_h2_input"/>
          收款渠道： <g:select  onchange="return query()" name="merchantId" from="${bankChanelList}" optionKey="${{it.MERCHANTID}}" optionValue="${{it.BANKNAME+'-'+it.ALIAS_NAME+'-'+it.SERVICECODE}}" value="${params.merchantId}" noSelection="${['':'--请选择--']}" />
          <br/>
          <br/>
      ...
      <tr>
        ...
        <g:sortableColumn params="${params}"  property="batchChanelname" title="${message(code: 'tbPcInfo.tbPcDkChanelname.label', default: '收款通道')}"/>
          <g:sortableColumn params="${params}"  property="batchStatus" title="${message(code: 'tbPcInfo.tbPcAccamount.label', default: '批次收款状态')}"/>
          <g:sortableColumn params="${params}"  property="batchStyle" title="收款方式"/>
      ...
      <g:each in="${ptbPayBatchList}" status="i" var="ptbPayBatch">
        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
          ...
          <td>
              <g:if test="${bo.hasPerm(perm:Perm.NewAgCol_Batch_View){true}}" >
                 <g:link action="batchsDetails" params="[batchId:ptbPayBatch.id,tradeType:'S']" >
                 ${ptbPayBatch.id}
                </g:link>
              </g:if>
          ...
          <td>
            ...
            <td >
                <div id="chanelId${i}div" style="width:100%;display:none;">
                    ...
                    <br/>
                    <a id="autochanelId${i}" style="display:none" onclick="return changeChanelCommit('${ptbPayBatch.id}','chanelId${i}','auto')">保存(自动收款)</a>
                    &nbsp;&nbsp;
                    <a id="handlechanelId${i}" style="display:none" onclick="return changeChanelCommit('${ptbPayBatch.id}','chanelId${i}','handle')">保存(手动收款)</a>
                    &nbsp;&nbsp;&nbsp;&nbsp;
                ...
                <span id="chanelId${i}divchange">
                     <g:if test="${ptbPayBatch.batchStyle=='handle'&&ptbPayBatch.batchStatus=='0'}">
                         <bo:hasPerm perm="${Perm.NewAgCol_Batch_ModChannel}">(<a  onclick="javascript:changeChanel('chanelId${i}')">修改</a>)</bo:hasPerm>
                     </g:if>
                 </span>
            </td>
            ...
            <td>
                <g:if test="${ptbPayBatch.batchStyle=='auto'&& ptbPayBatch.batchStatus=='1'}">
                    %{--<bo:hasPerm perm="${Perm.NewAgCol_Batch_Auto_ReSend}"><a onclick='reSendForAuto(${ptbPayBatch.id})'>重发(自动收款)</a></bo:hasPerm>--}%
                </g:if>
                <g:if test="${ptbPayBatch.batchStyle=='handle'}">
                    <g:if test="${ptbPayBatch.batchStyle=='handle'&&ptbPayBatch.batchStatus=='0'}">
                        %{--<bo:hasPerm perm="${Perm.NewAgCol_Batch_CANCEL}"><a onclick="cancelBatch(${ptbPayBatch.id})" >撤消</a></bo:hasPerm>--}%
                        <bo:hasPerm perm="${Perm.NewAgCol_Batch_CONFIRM}"><a onclick="confirmHandle(${ptbPayBatch.id},'chanelId${i}')" >确认</a></bo:hasPerm>
                    </g:if>
                    <g:if test="${ptbPayBatch.batchStatus=='0'||ptbPayBatch.batchStatus=='1'}">
                        <bo:hasPerm perm="${Perm.NewAgCol_Batch_Dl}"><g:link controller="ptbPayManager" action="downLoad" id="${ptbPayBatch.id}" > 下载</g:link></bo:hasPerm>
                    </g:if>
                </g:if>
                <g:if test="${ptbPayBatch.batchStatus=='2'}">
                    <bo:hasPerm perm="${Perm.NewAgCol_Trade_ReSendByBatch}"><a onclick="reSendCheckMsgByBatch('${ptbPayBatch.id}');">重发通知(批量)</a></bo:hasPerm>
                </g:if>
            </td>
        </tr>
      </g:each>
