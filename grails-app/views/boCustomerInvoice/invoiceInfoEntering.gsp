...
<div class="main">
  ...
  <div class="right_top">
    ...
    <form id="submitForm" action="invoiceInfoEntering">
    <h2>
    ...
    <h2>
        ...
        <bo:hasPerm perm="${Perm.WithDraw_InV_EnterSave}"><input type="button" onclick="enter()" value="保存" class="rigt_button"></bo:hasPerm>
        <bo:hasPerm perm="${Perm.WithDraw_InV_Download02}"><input type="button" onclick="down()" value="下载发票" class="rigt_button"></bo:hasPerm>
    ...
    <table align="center" class="right_list_table" id="infoTable">
      <tr>
        ...
        <bo:hasPerm perm="${Perm.WithDraw_InV_Detail02}"><g:sortableColumn params="${params}"  property="detail" title="明细"/></bo:hasPerm>
      ...
      <g:each in="${listCm}" status="i" var="showInfo">
        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
          ...
          <td><input name="invoiceNo" value="${showInfo.invoice_no}" maxlength="64" style="width:120px" onpropertychange="onChk(${i});" oninput="onChk(${i});" onfocus="this.select();" onblur="value=value.replace(/[\']/g, '').replace(/[' ']/g, '');" onkeyup="value=value.replace(/[\']/g, '').replace(/[' ']/g, '')" onbeforepaste="clipboardData.setData('text',clipboardData.getData( 'text').replace(/[\']/g, ''))"/></td>

          <td>
              <g:textField id="invoiceTime${i}" name="invoiceTime" value="${showInfo.invoice_time}" onchange="onChk(${i});" onblur="value=value.replace(/[ ]/g,'')" size="10" class="right_top_h2_input" style="width:80px"/>
                <script type="text/javascript">
                    $(function() {
                        $("#invoiceTime${i}").datepicker({ dateFormat: 'yy-mm-dd', changeYear: true, changeMonth: true });
                    });
                </script>
          </td>
          ...
          <bo:hasPerm perm="${Perm.WithDraw_InV_Detail02}"><td><input type="button" value="明细" onclick="detailWinOpen(${showInfo.id})"/></td></bo:hasPerm>
...
<script type="text/javascript">
    ...
    // 发票单号唯一性检查
    function chkInvNo(invoiceNo, id, no) {
        var flag = 0;
        var info = "";
        var nod = "";
        ...
        for (j = 0;  j < len; j++) {
            if (j != no) {
                info = info + nod + document.getElementsByName("invoiceNo")[j].value;
                nod = ",";
            }
        }
        if (info.indexOf(invoiceNo) > -1) {
            flag = 1;
        } else {
            var baseUrl = "${createLink(controller:'boCustomerInvoice', action:'chkInvoiceNo')}";
            $.ajax({
                url: baseUrl,
                type: 'post',
                async:false,
    ...
    function enter() {
        ...
        for (i = 0;  i < len; i++) {
            if (document.getElementsByName("checkBoxes")[i].checked) {
                ...
                var invoiceNo = document.getElementsByName("invoiceNo")[i].value;
                var invoiceTime = document.getElementsByName("invoiceTime")[i].value;
                if ((invoiceNo == null) || (invoiceNo == "")) {
                    alert("请输入发票号");
                    document.getElementsByName("invoiceNo")[i].focus();
                    return;
                } else if ((invoiceTime == null) || (invoiceTime == "")) {
                    alert("请输入发票开具时间");
                    document.getElementsByName("invoiceTime")[i].focus();
                    return;
                } else if (chkInvNo(invoiceNo, id, i) == 1) {
                    alert("发票号在页面中存在重复，请检查后重新输入");
                    document.getElementsByName("invoiceNo")[i].focus();
                    return;
                } else if (chkInvNo(invoiceNo, id, i) == 2) {
                    alert("发票号和数据库中的数据重复，请检查后重新输入");
                    document.getElementsByName("invoiceNo")[i].focus();
                    return;
                } else {
                    permInfo = permInfo + node + id + "thisIsASubLine" + invoiceNo + "thisIsASubLine" + invoiceTime;
                    node = "thisIsALine";
                }
            }
        }
        if (permInfo == "") {
            ...
            submitForm.action = "invoiceInfoEnterSave";
            submitForm.submit();
        }
    }
