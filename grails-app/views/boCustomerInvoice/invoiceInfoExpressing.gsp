...
<div class="main">
  ...
  <div class="right_top">
    ...
    <form id="submitForm" action="invoiceInfoExpressing">
    <h2>
    ...
    <h2>
        ...
        <bo:hasPerm perm="${Perm.WithDraw_InV_ExpressSave}"><input type="button" onclick="enter()" value="保存" class="rigt_button"></bo:hasPerm>
        <bo:hasPerm perm="${Perm.WithDraw_InV_Download03}"><input type="button" onclick="down()" value="下载发票" class="rigt_button"></bo:hasPerm>
    ...
    <table align="center" class="right_list_table" id="infoTable">
      <tr>
        ...
        <bo:hasPerm perm="${Perm.WithDraw_InV_Detail03}"><g:sortableColumn params="${params}"  property="detail" title="明细"/></bo:hasPerm>
      ...
      <g:each in="${listCm}" status="i" var="showInfo">
        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
          ...
          <td><input name="expressNo" value="${showInfo.express_no}" maxlength="64" style="width:120px" onpropertychange="onChk(${i});" oninput="onChk(${i});" onfocus="this.select();" onblur="value=value.replace(/[\']/g, '').replace(/[' ']/g, '');" onkeyup="value=value.replace(/[\']/g, '').replace(/[' ']/g, '')" onbeforepaste="clipboardData.setData('text',clipboardData.getData( 'text').replace(/[\']/g, ''))"/></td>

          <bo:hasPerm perm="${Perm.WithDraw_InV_Detail03}"><td><input type="button" value="明细" onclick="detailWinOpen(${showInfo.id})"/></td></bo:hasPerm>
...
<script type="text/javascript">
    ...
    // 发票单号唯一性检查
    function chkExpNo(expressNo, id, no) {
        var flag = 0;
        var info = "";
        var nod = "";
        ...
        for (j = 0;  j < len; j++) {
            if (j != no) {
                info = info + nod + document.getElementsByName("expressNo")[j].value;
                nod = ",";
            }
        }
        if (info.indexOf(expressNo) > -1) {
            flag = 1;
        } else {
            var baseUrl = "${createLink(controller:'boCustomerInvoice', action:'chkExpressNo')}";
            $.ajax({
                url: baseUrl,
                type: 'post',
                async:false,
                dataType: 'String',
                data:'id='+id+'&expressNo='+expressNo,
    ...
    function enter() {
        ...
        for (i = 0;  i < len; i++) {
            if (document.getElementsByName("checkBoxes")[i].checked) {
                ...
                var expressNo = document.getElementsByName("expressNo")[i].value;
                if ((expressNo == null) || (expressNo == "")) {
                    alert("请输入快递号");
                    document.getElementsByName("invoiceNo")[i].focus();
                    return;
                } else if (chkExpNo(expressNo, id, i) == 1) {
                    alert("快递号在页面中存在重复，请检查后重新输入");
                    document.getElementsByName("expressNo")[i].focus();
                    return;
                } else if (chkExpNo(expressNo, id, i) == 2) {
                    alert("快递号和数据库中的数据重复，请检查后重新输入");
                    document.getElementsByName("expressNo")[i].focus();
                    return;
                } else {
                    permInfo = permInfo + node + id + "thisIsASubLine" + expressNo;
                    node = "thisIsALine";
                }
            }
        }
        if (permInfo == "") {
            ...
            submitForm.action = "invoiceInfoExpressSave";
            submitForm.submit();
        }
    }
