...
<div class="main">
  ...
  <div class="right_top">
    ...
    <form id="submitForm" action="invoiceInfoShow">
    <h2>
    ...
    <h2>
        ...
        发票状态：<g:select name="status" from="${BoCustomerInvoice.statusMap}" optionKey="key" optionValue="value" value="${params.status}" class="right_top_h2_input"/>
    ...
    <h2>
        ...
        <bo:hasPerm perm="${Perm.WithDraw_InV_Download01}"><input type="button" onclick="down()" value="下载发票" class="rigt_button"></bo:hasPerm>
        ...
        <input type="hidden" id="factStatus" name="factStatus" value="${params.factStatus}"/>
    </h2>
    </form>
    <table align="center" class="right_list_table" id="infoTable">
      <tr>
        ...
        <g:sortableColumn params="${params}"  property="canceled_reason" title="退回原因"/>

        <bo:hasPerm perm="${Perm.WithDraw_InV_Detail01}"><g:sortableColumn params="${params}"  property="detail" title="明细"/></bo:hasPerm>
      ...
      <g:each in="${listCm}" status="i" var="showInfo">
        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
          ...
          <td title="${showInfo.canceled_reason}">
              <g:if test="${String.valueOf(showInfo.canceled_reason).length()>10}">${String.valueOf(showInfo.canceled_reason).substring(0,10)}...</g:if>
              <g:else>${showInfo.canceled_reason}</g:else>
          </td>

          <bo:hasPerm perm="${Perm.WithDraw_InV_Detail01}"><td><input type="button" value="明细" onclick="detailWinOpen(${showInfo.id})"/></td></bo:hasPerm>
...
<script type="text/javascript">
    ...
    function doSelect() {
        if ((document.getElementById("minAmount").value != '')&&(document.getElementById("maxAmount").value != '')&&(document.getElementById("minAmount").value > document.getElementById("maxAmount").value)) {
            ...
            submitForm.action = "invoiceInfoShow";
            submitForm.submit();
        }
    }
    ...
    function down() {
        ...
        for (i = 0;  i < len; i++) {
            if ((document.getElementsByName("checkBoxes")[i].checked)&&(document.getElementsByName("checkBoxes")[i].disabled == false)) {
                if (document.getElementById("factStatus").value == '1') {
