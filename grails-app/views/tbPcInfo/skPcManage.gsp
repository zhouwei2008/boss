    ...
    <table align="center" class="right_list_table" id="test">
      ...
      <g:each in="${tbPcInfoInstanceList}" status="i" var="tbPcInfoInstance">
        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
          ...
          <td>
              <g:if test="${bo.hasPerm(perm:Perm.AgCol_Batch_View){true}}">
                <g:link action="skitemShow" id="${tbPcInfoInstance.id}" >
                 ${tbPcInfoInstance.id}
                </g:link>
              </g:if>
...
%{--//               ${fieldValue(bean: tbPcInfoInstance, field: "tbPcFee")}--}%
            ...
            <td>${fieldValue(bean: tbPcInfoInstance, field: "tbPcDkChanelname")}</td>
             ...
             <td> <bo:hasPerm perm="${Perm.AgCol_Batch_ModChannel}"><g:link action="skmodify" id="${tbPcInfoInstance.id}" > 修改</g:link></bo:hasPerm></td>

             <td>${TbPcInfo.dkStatusMap[tbPcInfoInstance.tbPcDkStatus]}</td>

            <td><bo:hasPerm perm="${Perm.AgCol_Batch_Auto}"><a onclick='javascript:alert("此功能尚未开通")'>自动打款</a></bo:hasPerm></td>
             <g:hiddenField name="sk" id="sk" value="sk"/>
            <td><bo:hasPerm perm="${Perm.AgCol_Batch_Manual}"><g:link action="handledk" id="${tbPcInfoInstance.id}" params="[sk:'sk']">手动打款</g:link></bo:hasPerm></td>

             <td> <bo:hasPerm perm="${Perm.AgCol_Batch_Dl}"><g:link action="downLoad" id="${tbPcInfoInstance.id}" > 下载</g:link></bo:hasPerm></td>
