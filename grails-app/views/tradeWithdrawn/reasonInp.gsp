...
<head>
    ...
    <g:set var="entityName" value="${message(code: '退款原因输入', default: '退款原因输入')}"/>
...
<script type="text/javascript">
    function checkReason(id, flag) {
        ...
        if (flag == 0) {
            //提现处理
            window.parent.location.href = '${createLink(controller:'tradeWithdrawn', action:'checkRefused', params:['flag':'0'])}&id=' + id + '&note=' + note;
        } else if (flag == 1) {
            //单笔，单笔审批拒绝
            window.parent.location.href = '${createLink(controller:'tradeWithdrawn', action:'failOrPassRefuse', params:['flag':'1'])}&id=' + id + '&note=' + note;
        } else if (flag == 2) {
            //复核审批拒绝
            window.parent.location.href = '${createLink(controller:'tradeWithdrawn', action:'reCheckRefused', params:['flag':'2'])}&id=' + id + '&note=' + note;
        } else if (flag == 3) {
            //待处理审批拒绝
            window.parent.location.href = '${createLink(controller:'tradeWithdrawn', action:'firstCheckUnPass', params:['flag':'3'])}&id=' + id + '&note=' + note;
        }
