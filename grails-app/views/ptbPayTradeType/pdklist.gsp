...
<head>
  ...
  <g:set var="entityName" value="${message(code: '打款交易类型管理', default: '打款交易类型管理')}"/>
...
<div class="main">
  ...
  <div class="right_top">
    ...
    <h2>
      <g:form>
        <bo:hasPerm perm="${Perm.NewAgPay_PayTradeType_Proc}"><g:actionSubmit class="right_top_h2_button_tj" action="pdkcreate" value="${message(code: '创建 交易类型', default: '创建 交易类型')}"/></bo:hasPerm>
      </g:form>
    </h2>
    <table align="center" class="right_list_table" id="test">
      ...
      <g:each in="${ptbPayTradeTypeList}" status="i" var="ptbPayTradeType">
        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
          ...
          <td>
              <bo:hasPerm perm="${Perm.NewAgPay_PayTradeType_Edit}">
                  <g:link action="updateItem" params="[payId:ptbPayTradeType.id]">编辑</g:link>
              </bo:hasPerm>
              <bo:hasPerm perm="${Perm.NewAgPay_PayTradeType_Del}">
